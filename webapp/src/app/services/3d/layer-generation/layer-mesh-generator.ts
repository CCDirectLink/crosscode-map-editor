...
export class LayerMeshGenerator {
	public generateLevel(
		collLayer: CCMapLayer,
		aboveLayer: CCMapLayer | undefined,
		scene: Scene,
	) {
		...
		const levelOffset = getLevelOffsetTile(
			adjustLevel(collLayer.details.level),
		);
		const levelOffset2 = getLevelOffsetTile(
			adjustLevel(collLayer.details.level + 1),
		);
		...
		const grid = new NodeGrid(
			simpleTileLayer.width,
			simpleTileLayer.height,
		);
	...
	public transformToBelowMaster(layer: SimpleTileLayer, above?: CCMapLayer) {
		...
		for (const row of layer.tiles) {
			for (const tile of row) {
				let aboveIndex = -1;
				if (above) {
					const aboveTile = above
						.getPhaserLayer()
						.getTileAt(tile.x, tile.y + tileOffset);
					if (aboveTile) {
	...
	private generateMesh(
		...
		ccLayer: CCMapLayer,
		simpleTileLayer: SimpleTileLayer,
		scene: Scene,
	) {
		...
		if (level + 1 >= Globals.map.masterLevel) {
			heightOffset =
				getLevelOffsetTile(adjustLevel(level + 1)) -
				getLevelOffsetTile(adjustLevel(level));
		} else {
			heightOffset =
				getLevelOffsetTile(adjustLevel(level + 2)) -
				getLevelOffsetTile(adjustLevel(level + 1));
		}

		let offsetY =
			(layer.tilemap.height - maxY + heightOffset) / layer.tilemap.height;
		...
		const top = MeshBuilder.CreatePolygon(
			name,
			{
				shape: pathArr,
				holes: holes,
		...
		const mesh = sideMeshGenerator.generate(
			pathArr,
			holes,
			ccLayer,
			simpleTileLayer,
		...
		const horizontalOffset = getLevelOffsetTile(
			adjustLevel(ccLayer.details.level),
		);
