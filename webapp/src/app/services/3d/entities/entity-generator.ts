...
export class EntityGenerator {
	...
	private generatePlane(
		...
		for (let start = 0; start < fullWidth; start += width) {
			...
			const uvs = this.calculateUvs(
				material.diffuseTexture,
				customFix,
				entity.entitySettings.scalableX,
			);
			...
			mesh.position.addInPlaceFromFloats(
				start + (currWidth - width) / 2,
				0,
				0,
			);
	...
	private generateBox(
		...
		const uvs = this.calculateBoxUvs(
			material.diffuseTexture,
			fix,
			size.y,
			fix.flipX,
		...
		mesh.position.copyFromFloats(
			entity.container.x / Globals.TILE_SIZE,
			pos.z,
			-entity.container.y / Globals.TILE_SIZE,
		);
		mesh.position.addInPlaceFromFloats(
			scaledSize.x / 2,
			scaledSize.z / 2,
	...
	private posFromEntity(entity: CCEntity) {
		...
		out.addInPlaceFromFloats(boundBoxOffset.x, boundBoxOffset.y, 0);
	...
	private async makeMaterial(entity: CCEntity, fix: Fix, scene: Scene) {
		...
		if (
			fix.renderMode === 'lighter' ||
			entity.entitySettings.sheets.renderMode === 'lighter'
		) {
	...
	private calculateBoxUvs(
		...
		const bottom = this.calculateUvs(
			texture,
			{
				x: fix.x,
				y: fix.y,
