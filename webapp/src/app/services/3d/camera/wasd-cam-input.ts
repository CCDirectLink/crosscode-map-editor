...
export class WasdCamInput implements ICameraInput<CustomFreeCamera> {
	...
	checkInputs() {
		if (this.onKeyDown) {
			...
			const scale = this.keysTurbo.some((t) => this.keys.has(t))
				? 3.5
				: 1;
			...
			for (const key of this.keys) {
				...
				camera
					.getViewMatrix()
					.invertToRef(camera._cameraTransformMatrix);
				Vector3.TransformNormalToRef(
					camera._localDirection,
					camera._cameraTransformMatrix,
					camera._transformedDirection,
				);
				camera._transformedDirection.scaleToRef(
					this.sensibility * scale,
					camera._transformedDirection,
				);
