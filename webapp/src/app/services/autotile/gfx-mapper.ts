...
export class GfxMapper {
	private AUTOTILE_CONFIG: Record<string, AutotileConfig[] | undefined> = {};

	private TILESET_CONFIG: Record<string, ChipsetConfig | undefined> = {};

	private mapping: Record<AutotileType, Map<number, keyof FillType>> =
		{} as any;
	...
	private async init() {
		this.TILESET_CONFIG =
			await this.jsonLoader.loadJsonMerged('tilesets.json');
	...
	private async generateAutotileConfig() {
		const jsons =
			await this.jsonLoader.loadJson<JsonType[]>('autotiles.json');
		...
		for (const config of autotilesJson) {
			...
			for (const autotile of config.autotiles) {
				const generatedType: AutotileType =
					`${autotile.size.x}x${autotile.size.y}` as AutotileType;
				...
				if (cliff === undefined) {
					for (const terrain of terrains) {
						if (
							terrain.ground.x === autotile.base.x &&
							terrain.ground.y === autotile.base.y
						) {
							cliff = terrain.cliff;
							break;
						}
	...
	getAutotileConfig(
		...
		tile: number,
		checkCliffs = false,
	): { config: AutotileConfig; isCliff: boolean } | null {
		...
		for (const config of configs) {
			...
			if (
				checkCliffs &&
				this.getFill(pos, config.cliff, this.cliffBorderMapping)
			) {
	...
	private getFill(
		...
		mapping: Map<number, keyof FillType>,
	) {
		if (!offset) {
			return undefined;
		}
	...
	getGfx(fillType: keyof FillType, config: AutotileConfig) {
		...
		return this.getTile(
			config.base.x + offset.x,
			config.base.y + offset.y,
			config.tileCountX,
		);
	}
