...
import {
	...
	CHECK_ITERATE,
	CheckDir,
} from '../height-map/heightmap.constants';
...
export class AutotileService {
	...
	constructor() {
		...
		combineLatest([
			...
			for (const tile of tiles) {
				const config = this.gfxMapper.getAutotileConfig(
	...
	public drawTile(
		...
		const tilesToUpdate = CHECK_ITERATE.map((v) =>
			this.getOther(config.config, layer, tileData, CHECK_DIR[v]),
	...
	private getOther(
		...
		if (
			newPos.x < 0 ||
			newPos.y < 0 ||
			newPos.x >= layer.details.width ||
			newPos.y >= layer.details.height
		) {
			out.fill = 'XXXX';
			out.update = false;
			return out;
		...
		const index = layer
			.getPhaserLayer()
			.getTileAt(newPos.x, newPos.y, true).index;
		if (index === 0) {
		...
		if (fill) {
			...
			out.update = false;
			out.fill =
				this.gfxMapper.getFillType(config, index, true) || out.fill;
		}
