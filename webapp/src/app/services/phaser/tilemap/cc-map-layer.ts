...
export class CCMapLayer {
	...
	async updateTileset(tilesetname: string) {
		...
		const newTileset = this.tilemap.addTilesetImage(
			tilesetname,
			undefined,
			undefined,
			undefined,
	...
	updateLevel(level?: number | string) {
		if (level === undefined) {
			level = (this.details.levelName ?? this.details.level) as
				| string
				| number;
		}
	...
	private makeLayer(
		tileset?: string | string[] | Phaser.Tilemaps.Tileset,
		tiles?: Tile[][] | number[][],
	) {
