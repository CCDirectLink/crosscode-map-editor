...
export class Enemy extends DefaultEntity {
	protected override async setupType(settings: EnemyAttributes) {
		...
		const enemyPath = PathResolver.convertToPath(
			BasePath.ENEMIES,
			settings.enemyInfo.type || '',
			FileExtension.NONE,
		);
		const enemyData = (await Helper.getJsonPromise(enemyPath)) as
			| EnemyData
			| undefined;
		if (!enemyData) {
		...
		const sheetPath = PathResolver.convertToPath(
			BasePath.ANIMATIONS,
			enemyData.anims,
			FileExtension.NONE,
		);
		...
		const sheet = this.resolveSUB(rawSheet) as
			| MultiDirAnimation[]
		...
		if (this.isMultiDir(sheet)) {
			...
			if (
				!(await this.renderMultiDirAnims(
					anims.length > 0 ? anims : [sheet[0]],
				))
			) {
				this.generateErrorImage();
				return;
	...
	private isMultiDir(
		sheet: MultiDirAnimation[] | [MultiEntityAnimation],
	): sheet is MultiDirAnimation[] {
		return sheet[0].DOCTYPE === 'MULTI_DIR_ANIMATION';
	}

	private async renderMultiDirAnims(
		anims: MultiDirAnimation[],
	): Promise<boolean> {
		const results = await Promise.all(
			anims.map((anim) => this.renderMultiDirAnim(anim)),
		);
		return results.every((r) => r); //All results true
	}

	private async renderMultiDirAnim(
		anim: MultiDirAnimation,
	): Promise<boolean> {
		const tileSheet =
			typeof anim.sheet === 'string'
				? anim.namedSheets[anim.sheet]
				: anim.sheet;
		...
		const tileOffset =
			anim.dirs && anim.tileOffsets
				? anim.tileOffsets[Math.floor(anim.dirs / 2)]
				: 0;
	...
	private render(anim: TileSheet, pos: Point, frame: number): void {
		const offsetX =
			(anim.offX || 0) +
			(anim.xCount ? (frame % anim.xCount) * anim.width : 0);
		const offsetY =
			(anim.offY || 0) +
			(anim.xCount ? Math.floor(frame / anim.xCount) * anim.height : 0);
	...
	private async renderMultiEntityAnim(
		animation: MultiEntityAnimation,
	): Promise<boolean> {
		const partAnims: Record<
			string,
			Record<string, SingleDirAnimation[]>
		> = {};
	...
	private renderSingleDirAnim(
		anim: SingleDirAnimation,
		offset: Point,
		size: Point3,
	): void {
		...
		const offsetY =
			(sheet.offY || 0) +
			(sheet.xCount
				? Math.floor(anim.frames[0] / sheet.xCount) * sheet.height
				: 0);
