...
export class BaseTileDrawer extends BaseObject {
	...
	protected override init(): void {
		this.previewTileMap = this.scene.add.tilemap(
			undefined,
			Globals.TILE_SIZE,
			Globals.TILE_SIZE,
		);
	...
	preUpdate(): void {
		...
		if (this.rightClickStart) {
			...
			if (
				this.rightClickEnd &&
				this.rightClickEnd.x === p.x &&
				this.rightClickEnd.y === p.y
			) {
	...
	protected override activate(): void {
		...
		const pointerDown = (pointer: Phaser.Input.Pointer) => {
			if (
				...
				(this.leftClick && pointer.leftButtonDown())
			) {
		...
		const pointerUp = (pointer: Phaser.Input.Pointer) => {
			if (
				(pointer.rightButtonReleased() ||
					(this.leftClick && pointer.leftButtonReleased())) &&
				this.rightPointerDown
			) {
				this.onMouseRightUp();
			}
		};
		...
		this.addSubscription(
			Globals.phaserEventsService.changeSelectedTiles.subscribe((tiles) =>
				this.updateSelectedTiles(tiles),
			),
		);
	}
	...
	public async setLayer(layer?: CCMapLayer) {
		...
		const tileset = this.previewTileMap.addTilesetImage(
			'only',
			layer.details.tilesetName,
		);
		if (tileset) {
	...
	private onMouseRightUp() {
		...
		const tilesWithin = phaserLayer.getTilesWithin(
			smaller.x,
			smaller.y,
			width,
			height,
			...
			makeText(
				{
					x:
						Globals.TILE_SIZE / 2 +
						(width > 0 ? 0 : width * Globals.TILE_SIZE),
	...
	private renderPreview() {
		...
		const layer = this.previewTileMap.createBlankLayer(
			'layer',
			'only',
			0,
			0,
