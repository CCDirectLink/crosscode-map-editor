...
export class Helper {
	...
	public static isInBounds(layer: CCMapLayer, p: Point): boolean {
		return (
			p.x >= 0 &&
			p.y >= 0 &&
			p.x < layer.details.width &&
			p.y < layer.details.height
		);
	}
	...
	public static getJson(key: string, callback: (json: any) => void) {
		...
		Globals.httpService.resolveFile(key + '.json').subscribe(
			(file) => {
				// load json
	...
	public static async loadTexture(
		...
		const file = await Globals.httpService
			.resolveFile(key)
			.toPromise()
			.catch(() => false);
		if (!file) {
	...
	public static getMapStyle(
		map: CCMap,
		type: keyof MapStyles,
	): MapStyle | undefined {
		const mapStyles =
			Globals.jsonLoader.loadJsonMergedSync<MapStyles>('map-styles.json');
