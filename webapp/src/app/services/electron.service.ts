...
export class ElectronService implements SharedService {
	...
	public static async init() {
		...
		ElectronService.assetsPath =
			localStorage.getItem(ElectronService.storageName) || '';
	...
	private static async updateMod() {
		await api.selectedMod(
			ElectronService.assetsPath,
			ElectronService.selectedMod,
		);
	}
	...
	public selectCcFolder(): string | undefined {
		...
		const newPath = this.remote.dialog.showOpenDialogSync({
			...
			defaultPath:
				'C:\\Program Files (x86)\\Steam\\steamapps\\common\\CrossCode\\assets',
