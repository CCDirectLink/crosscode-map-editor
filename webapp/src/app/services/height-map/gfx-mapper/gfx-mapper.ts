...
export class GfxMapper {
	...
	private copySettings(mappingSettings: ChipsetBase) {
		const result: ChipsetMapping = JSON.parse(
			JSON.stringify(mappingSettings),
		);
	...
	getChasmHeight(terrain: number) {
		...
		if (mapping.mapping['CHASM'].wallYVariance) {
			minHeight =
				mapping.mapping['CHASM'].wallYVariance[GFX_TYPE.WALL_SOUTH]
					.start.length;
		}
		return minHeight;
	}
	...
	isFill(gfxType: GFX_TYPE, terrain?: number) {
		return (
			gfxType === GFX_TYPE.FILL ||
			(terrain && this.terrains[terrain - 1].blockedTypes
				? this.terrains[terrain - 1].blockedTypes!.indexOf(gfxType) !==
					-1
				: this.base.blockedTypes &&
					this.base.blockedTypes.indexOf(gfxType) !== -1)
		);
	}
	...
	isWallTerrainFromTop(lower: number, upper: number) {
		...
		return (
			((upperTerrain && upperTerrain.wallTerrainPrio) || 0) >
			((lowerTerrain && lowerTerrain.wallTerrainPrio) || 0)
		);
	}
	...
	getGfx(
		...
		x: number,
		y: number,
		subType: keyof GfxMaps | null,
		terrain: number,
		terrainBorder = -1,
		wallProps?: { start: number; end: number },
	) {
		if (terrain && !this.terrains[terrain - 1]) {
			terrain = 0;
		}
		...
		const ignore =
			subMapping && subMapping.overrideWallBase
				? mapping['SUB'].ignoreTerrainKeepWallBase
				: mapping['SUB'].ignoreTerrain;
		...
		if (subMapping) {
			// @ts-expect-error sub not considered for type
		...
		if (wallProps && wallYVariance && wallYVariance[gfxType]) {
			...
			if (variance.end && wallProps.end < variance.end.length) {
				...
				variance.start &&
				wallProps.start < variance.start.length
			) {
