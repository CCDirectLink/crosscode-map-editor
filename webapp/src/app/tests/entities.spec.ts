...
describe('Entities', () => {
	...
	it('make new npc and export map', async () => {
		...
		expect(exported2.entities.length - exported1.entities.length).toBe(
			1,
			'exported map should have 1 additional entity',
		);
	});
	...
	it('copy paste entity', async () => {
		...
		expect(exported2.entities.length - exported1.entities.length).toBe(
			1,
			'copy paste should add 1 entity',
		);
...
function checkMapId(exported: CrossCodeMap) {
	...
	expect(new Set(ids).size).toBe(
		...
		'mapId should be unique for every entity',
	);
}
