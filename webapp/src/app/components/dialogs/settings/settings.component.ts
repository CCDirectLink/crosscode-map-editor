...
export class SettingsComponent implements OnInit {
	...
	ngOnInit() {
		if (this.isElectron) {
			...
			this.folderFormControl.valueChanges.subscribe(() =>
				this.resetIcon(),
			);
		}
	...
	check() {
		const valid = this.electron.checkAssetsPath(
			this.folderFormControl.value,
		);
		this.setIcon(valid);
		if (valid) {
	...
	save() {
		...
		void this.sharedService.saveModSelect(this.mod);
		this.settingsService.updateSettings(this.settings);
		this.close();
		const ref = this.snackBar.open(
			'Changing the path requires to restart the editor',
			'Restart',
			{
				duration: 6000,
			},
