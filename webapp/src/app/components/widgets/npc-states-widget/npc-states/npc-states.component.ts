...
export class NpcStatesComponent implements OnInit {
	...
	selectTab(index: number) {
		if (this.currentState) {
			...
			this.currentState.event = createEventArray(
				this.eventEditor.export(),
				this.eventType,
				this.trader,
			);
		...
		this.index = index;
		if (this.currentState) {
			//Undefined if this.states.length is 0
			({ type: this.eventType, trader: this.trader } =
				destructureEventArray(this.currentState.event));
		}
	}
	...
	updateDisplayedWarnings() {
		...
		if (this.isTradeEvent) {
			...
			if (this.missingTradeEvent) {
				this.warnings.push(
					'START_NPC_TRADE_MENU event is missing, trade popup will not appear',
				);
			}
		}
	}
	...
	updateTradeEventWarning(updatedEvents: EventType[]) {
		function hasEventOfTypeRecursive(object: any, type: string): boolean {
			...
			for (const [key, value] of Object.entries(object)) {
				if (
					(key === 'type' && value === type) ||
					hasEventOfTypeRecursive(value, type)
				) {
					return true;
				}
			}
