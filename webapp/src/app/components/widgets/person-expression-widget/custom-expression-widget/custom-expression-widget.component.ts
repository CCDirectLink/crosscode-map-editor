...
import {
	ExpressionRendererEntity,
	ExpressionRendererSettings,
} from './expression-renderer-entity';

@Component({
	selector: 'app-custom-expression-widget',
	templateUrl: './custom-expression-widget.component.html',
	styleUrls: [
		'./custom-expression-widget.component.scss',
		'../../widget.scss',
	],
	standalone: false,
})
export class CustomExpressionWidgetComponent
	extends OverlayWidget<Person>
	implements OnChanges
{
	...
	private async getFace() {
		...
		let sheet =
			((await Helper.getJsonPromise('data/characters/' + person)) as
				| CharacterSettings
				| undefined) ?? {};
		...
		if (typeof sheet.face === 'object' && sheet.face?.ABSTRACT) {
			const abstractFaces = await this.jsonLoader.loadJsonMerged<
				Record<string, Face>
			>('abstract-faces.json');
	...
	private async makeImage(face: Face, name: string): Promise<string> {
		const entity = new ExpressionRendererEntity(
			Globals.scene,
			Globals.map,
			-999999,
			0,
