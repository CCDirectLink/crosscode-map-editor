...
import {
	...
	GfxEndsDir,
	ScalableProp,
	ScalablePropAttributes,
	ScalablePropConfig,
	ScalablePropDef,
} from '../../../services/phaser/entities/registry/scalable-prop';
import { Helper } from '../../../services/phaser/helper';
import {
	prepareSheet,
	ScalablePropSheet,
} from '../../../services/phaser/sheet-parser';
...
@Component({
	...
	styleUrls: [
		'./scalable-prop-config-widget.component.scss',
		'../widget.scss',
	],
	standalone: false,
})
export class ScalablePropConfigWidgetComponent
	extends OverlayWidget<ScalablePropAttributes>
	implements OnInit, OnDestroy
{
	...
	private async updateProps() {
		...
		let sheet = (await Helper.getJsonPromise(
			'data/scale-props/' + sheetPath,
		)) as ScalablePropSheet;
		if (!sheet) {
		...
		this.comp.leftGroup.props = await Promise.all(
			entries.map(async (prop) => {
				return {
					name: prop.name,
	...
	private async updateRightGroups() {
		...
		for (const [k, v] of Object.entries(def.gfxEnds ?? {})) {
			...
			for (const patternName of Object.keys(value)) {
				props.push({
					name: patternName,
					imgSrc: await this.generateImg(def, name, {
						[key]: patternName,
					}),
				});
			}
	...
	private async updatePreview() {
		...
		this.comp.preview = await this.generateImg(
			def,
			propConfig.name,
			propConfig.ends,
			true,
