...
export class AddEventService {
	...
	private async init() {
		const events =
			await this.jsonLoader.loadJsonMerged<EventsJson>('events.json');
		const actions =
			await this.jsonLoader.loadJsonMerged<ActionsJson>('actions.json');
	...
	select(event: string) {
		...
		const instance: AbstractEvent<any> = new clss(
			this.domSanitizer,
			{ type: event },
			this.actionStep,
		);
