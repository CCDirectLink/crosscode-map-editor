...
import { PropAttributes } from '../../../services/phaser/entities/registry/prop';
...
export class PropTypeWidgetComponent extends OverlayWidget<PropAttributes> {
	...
	private async updateProps() {
		...
		let sheet = (await Helper.getJsonPromise(
			'data/props/' + sheetPath,
		)) as PropSheet;
		sheet = prepareSheet(sheet);

		const propDefs = sheet.props.filter((v) => v.name) as (PropDef & {
			name: string;
		})[];
		this.comp.showRightProps = false;

		await Promise.all(
			propDefs.map(async (def) => {
