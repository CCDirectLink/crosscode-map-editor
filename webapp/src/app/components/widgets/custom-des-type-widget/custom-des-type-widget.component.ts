...
import {
	ItemDestructAttributes,
	ItemDestructTypes,
} from '../../../services/phaser/entities/registry/item-destruct';
...
export class CustomDesTypeWidgetComponent extends OverlayWidget<ItemDestructAttributes> {
	...
	private async updateProps() {
		...
		if (this.comp.global) {
			...
			destructibles = Object.values(settings.ENTITY.ItemDestruct).map(
				(v) => ({
		...
		await Promise.all(
			destructibles.map(async (destructible) => {
				props.push({
					name: destructible.name,
					imgSrc: await this.generateImage<ItemDestructAttributes>(
						{ desType: destructible.desType },
						'ItemDestruct',
					),
				});
			}),
