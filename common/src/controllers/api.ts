...
const mods: string[] = [];
let packagesCache: Record<
  string,
  {
    folderName: string;
    displayName: string;
    ccmodDependencies?: Map<string, string>;
  }
>;
...
function selectMod(
  ...
  packages: Record<
    string,
    { folderName: string; ccmodDependencies?: Map<string, string> }
  >,
  result: string[],
) {
...
async function readMods(dir: string) {
  ...
  const ccmodFolderNames = new Set(
    filesCCMod.map((file) => path.basename(path.dirname(file))),
  );
  ...
  for (const [name, pkg] of rawPackages) {
    try {
      ...
      packages[parsed.name] = {
        ...
        ccmodDependencies:
          parsed.ccmodDependencies ?? parsed.dependencies ?? {},
      };
    } catch (err) {
...
export async function getAllFiles(dir: string) {
  ...
  for (const mod of mods) {
    ...
    await listAllFiles(
      path.resolve(modDir, "media/"),
      images,
      "png",
      path.resolve(modDir),
    );
    await listAllFiles(
      path.resolve(modDir, "data/"),
      data,
      "json",
      path.resolve(modDir),
    );
...
export async function getAllModMapEditorConfigs(
  dir: string,
): Promise<ModEditorConfig[]> {
  const packages = await readMods(dir);
